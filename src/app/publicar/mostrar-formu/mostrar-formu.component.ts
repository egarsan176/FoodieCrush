...
@Component({
  selector: 'app-mostrar-formu',
  templateUrl: './mostrar-formu.component.html',
  styleUrls: ['./mostrar-formu.component.css']
})
export class MostrarFormuComponent implements OnInit {
    ...
    this.uploadService.getFileByName().subscribe({
      next: (data =>{
        ...
        if(this.recipe.recipeName!="" && this.recipe.method.length>0 && this.recipe.category!=0 && this.recipe.ingredientLine.length>0){
          ...
          this.recipeService.publicar(this.recipe)
          .subscribe({
            next: (data => {
              Swal.fire({
